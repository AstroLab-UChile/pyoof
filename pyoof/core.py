...
def residual_true_multifrequency(params, data, illum_func, telgeo, resolution, box_factor, interp):
    ...
    for key in data:
        if key != "pthto":
            ...
            #residuals.append(norm(current["beam_data"] - beam_model, axis=1))
...
def multifrequency_zernike_fit(data, order_max, illum_func, telescope, resolution,
    ...
    for order in range(1, order_max + 1):
        ...
        res_optim = res_lsq.fun.reshape(len(d_z), -1)  # Optimum residual solution
        ...
        for key in data:
            ...
            if key!= "pthto":
                ...
                u_data = current["u_data"]
                v_data = current["v_data"]
                wavel = str(current["wavel"].to_value())
                for i in range(len(d_z)):
                    signal_noise_ratio.append(np.round(snr(beam_data[i, ...], u_data[i, ...], v_data[i, ...]), 2))
                pyoof_info = dict(
                    tel_name=tel_name,
                ...
                print(beam_data[:,0:u_data.shape[0]].shape)
                print(res_optim.shape)
                print(jac_optim.shape)
                print(grad_optim.shape)

                save_to_csv = [
                    beam_data[0:u_data.shape[0]], u_data.to_value(apu.rad), v_data.to_value(apu.rad),
                    res_optim[0:u_data.shape[0]], jac_optim, grad_optim, _phase, cov_ptrue, cor_ptrue
                    ]
